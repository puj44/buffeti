...
const SuggestivePackage = dynamic(
  () => import("@/components/Homepage/SuggestivePackage"),
  {
    loading: () => (
...
export default function Home(props) {
  ...
  return (
    <div
      className={`flex flex-col md:gap-6 gap-4 py-8  page-spacing`}
    ...
    >
      ...
      <h4 className="font-semibold hidden page-title 2xl:text-center sm:block">
        Snack box And Mini Meals
      </h4>
...
async function fetchData(store, location) {
  const packageFilters = (
    await getFiltersApi({ payload: { menuOption: "click2cater", location } })
  )?.data;
  ...
  if (filters && filters?.categories) {
    if (Object.keys(filters?.categories).length > 0) {
      const payload = {
        menuOption:
          "click2cater?category=" + Object.keys(filters.categories)[0],
        location,
      };
  ...
  return {
    filters: filters ?? {},
    packages:
      Object.keys(packages?.packages ?? {})?.length > 0
