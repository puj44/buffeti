...
import PackageCardLoader from "../SkeletonLoader/PackageCardLoader";
...
function SuggestivePackage({ data, filters, noOfPeople }) {
  ...
  const handleChangeFilter = (field, val) => {
    ...
    if (val === activeFilters?.[field]) {
      if (field !== "category") delete filtersData[field];
    } else {
      filtersData = {
        ...filtersData,
        [field]: val,
  ...
  return (
    <div
      className="flex flex-col gap-4 md:gap-6  max-w-[1182px] w-full mx-auto  "
      id={"suggestive-package"}
    >
      ...
      <p className="hidden lg:block font-semibold sub-title pt-2">
        Select Your Package
      </p>
      {/* ALL PACKAGES CARD */}
      <div className="grid grid-flow-row sm:grid-cols-2 lg:grid-cols-3  justify-center md:justify-between w-full gap-4 mt-[-4px]">
        {packagesData &&
          ...
          Object.keys(packagesData).map((pd, idx) => {
            return (
              <PackageCard
                key={"package-" + pd}
                numberOfPeople={activeFilters?.no_of_people ?? "10_20"}
                data={packagesData[pd]}
              />
            );
          })}
      </div>
