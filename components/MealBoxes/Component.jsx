...
const PackageCard = dynamic(() => import("../Homepage/PackageCard"), {
  ...
  return (
    <div className="flex flex-col gap-6 page-spacing h-full py-6">
      ...
      <div className="grid grid-flow-row gap-6  h-full items-start">
        <h1 className="font-semibold sub-title">
          {mealBox === "click2cater"
            ? "Select Your Package"
            : "Select Your Mini Meal"}
        </h1>
        <div className="grid grid-flow-col gap-6 justify-self-start h-full items-start">
          ...
          <div className="flex flex-col gap-6 w-full h-full">
            ...
            {isLoading ? (
              ...
              <div className="grid grid-flow-row sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3  justify-between w-full gap-6">
                {packagesData?.[activeFilters?.category] &&
                Object.keys(packagesData?.[activeFilters?.category])?.length > 0
                  ? Object.keys(packagesData?.[activeFilters?.category]).map(
                      (pd, idx) => {
                        const pack =
                          packagesData?.[activeFilters?.category][pd];
                        return (
