...
function OrderTimePicker({
    ...
    const timeValue = (val) =>{
        ...
        const timeStamp = parseInt(split) >= 12 || parseInt(split) <= 10 ? "pm" :"am";
        return `${val} ${timeStamp}`;
    }
    console.log("HERE",timeValues);
  return (
       ...
       
    {
        <Dropdown 
            ...
            placement='bottom'
        >
            <Dropdown.Item as="div" className='flex flex-col py-3 px-3 max-h-[300px] overflow-y-scroll'>
                {
                    timeValues.map((tv,idx)=>{
                        return(
                            <div key={"time-"+idx} onClick={() => {handleChangeTime(tv)}} className='hover:bg-gray-200 p-1.5 rounded-md'>
                                {timeValue(tv)}
                            </div>
                        )
                    })
