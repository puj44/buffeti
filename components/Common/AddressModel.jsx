import { addAddress, editAddress } from '@/redux/reducers/addressReducer';
  ...
  const handleSubmit =()=>{
    ...
    fields.forEach((fd)=>{
      if(!addressData?.[fd.field]){
        isValid = false;
        errorMessages = {...errorMessages, [fd.field]:`Please Enter ${fd.title}`}
      }
    })
    ...
    if(!addressData?.lattitude  && !addressData?.longitude){
      if ("geolocation" in navigator) {
        ...
        isValid = false;
        setErrors({...errors, location:"Geolocation is not available in your browser, please try different browser"})
      }
    }
    if(isValid){
      setLoading(true);
            ...
            <div className='bg-white flex flex-col gap-5 justify-between overflow-y-scroll sm:overflow-y-visible py-6 px-5 relative w-full h-full md:h-fit md:max-w-[642px] md:max-h-[800px] my-auto  rounded-lg '>
              <div className='grid grid-flow-row   gap-5'>
                ...
                </div>
                {
                  ...
                  <span className='small-title' style={{color:"#475467"}}>{errors?.location}</span>
                }
                        ...
                        return(
                          ...
                          >
                            <div className='flex flex-col gap-1.5 '>
                                <label className='font-medium' style={{color:"#344054"}}>
                                  {fd.title}
                                </label>
                                {
                                 fd.fieldType === "input" ?
                                 <input  
                                   id={fd.field}
                                   onChange={(e)=>{ handleChangeInput(e, fd.field)}} 
                                   value={data?.[fd.field] ?? ""} 
                                 ...
                                 : fd.fieldType === "select" ?
                                  ...
                                  >
                                    <option selected disabled value="" className=''>{fd.title}</option>
                                    {
                                        locations?.map((loc)=>{
                                            return(
                            ...
                            {
                              (errors && errors?.[fd.field] && errors?.[fd.field] !== "") &&
              ...
              <div className=' flex flex-col sm:flex-row gap-3 w-full '>
                  ...
                  <button className='btn primary-btn order-1 sm:order-2 font-semibold w-full' onClick={()=>{handleSubmit()}}>
                    {
                      isLoading?
